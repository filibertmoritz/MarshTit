...
data <- data %>% 
  mutate(Effort.log = log(if_else(Effort == 0, 0.000000001, Effort)), 
         Effort.sqrt = sqrt(Effort))
...
formulas <- list(nullmodel = formula(occ ~ Effort.sqrt.scaled),
                 NMD.Deciduous = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled),
                 # NMD.Mixed.forest = formula(occ ~ NMD.Mixed.forest.scaled), 
                 NMD.Hardwood = formula(occ ~ Effort.sqrt.scaled + NMD.Hardwood.scaled), 
                 NMD.Evergreen = formula(occ ~ Effort.sqrt.scaled + NMD.Evergreen.scaled), # avoidance of decidous forest
                 `NMD.Hardwood + NMD.Deciduous` = formula(occ ~ Effort.sqrt.scaled + NMD.Hardwood.scaled + NMD.Deciduous.scaled))
...
formulas <- c(formulas, list(`NMD.Deciduous + NMD.Hardwood + LiDAR.bottom` = formula(occ ~ Effort.sqrt.scaled +  NMD.Deciduous.scaled + NMD.Hardwood.scaled + LiDAR.bottom.scaled),
                             `NMD.Deciduous + NMD.Hardwood + LiDAR.lower` = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled + NMD.Hardwood.scaled + LiDAR.lower.scaled), 
                             `NMD.Deciduous + NMD.Hardwood + LiDAR.upper` = formula(occ ~ Effort.sqrt.scaled +  NMD.Deciduous.scaled + NMD.Hardwood.scaled + LiDAR.upper.scaled),
                             `NMD.Deciduous + NMD.Hardwood + LiDAR.top` = formula(occ ~ Effort.sqrt.scaled +  NMD.Deciduous.scaled + NMD.Hardwood.scaled + LiDAR.top.scaled), 
                             `NMD.Deciduous + NMD.Hardwood + canopy.height` = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled + NMD.Hardwood.scaled + canopy.height.scaled)
                             ))
...
formulas <- c(formulas, list(`NMD.Deciduous + NMD.Hardwood + canopy.height + Moisture` = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled + NMD.Hardwood.scaled + canopy.height.scaled + Moisture.scaled),
                             `NMD.Deciduous + NMD.Hardwood + canopy.height + Moisture2` = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled + NMD.Hardwood.scaled + canopy.height.scaled + I(Moisture.scaled^2))))
...
formulas <- c(formulas, list(`NMD.Deciduous + NMD.Hardwood + canopy.height + Moisture2 + NMD.Open.artificial` = formula(occ ~ Effort.sqrt.scaled + NMD.Deciduous.scaled + NMD.Hardwood.scaled + canopy.height.scaled + I(Moisture.scaled^2) + NMD.Open.artificial.scaled)))
...
print(doc, target = "output/tables/Marsh.tit.model.select.table.effect.docx") # actually save the table
...
saveRDS(object = best.model.unscaled, "output/Marsh.tit.best.model.effort.rds")
...
testDispersion(simulateResiduals(best.model)) # re.form needed for conditional simulations, as recommended in https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html, 1.1 is alright - no overdispersion
...
print(doc, target = "output/tables/Marsh.tit.effect.sizes.effect.docx") # actually save the table
...
new.data <- data.frame(
  ...
  canopy.height = make.seq(data$canopy.height, n = n), 
  canopy.height.scaled = make.seq(data$canopy.height.scaled, n = n),
  ...
  Effort.sqrt = make.seq(data$Effort.sqrt, n = n), 
  Effort.sqrt.scaled = make.seq(data$Effort.sqrt.scaled, n = n))
...
plot <- plotting.data %>% # filter(!predictor == "Effort.sqrt") %>%
  mutate(predictor = case_when(predictor == 'canopy.height' ~ 'b) Canopy height',
                               predictor == "NMD.Deciduous" ~ "a) Softwood deciduous forest", 
                               ...
                               predictor == "Effort.sqrt" ~ "f) Observation effort (sqrt)")) %>% 
  ggplot() + 
  ...
  scale_fill_discrete(name= 'Units of the Predictors', 
                      labels =  c("Fraction of softwood \ndeciduous forest",  "Canopy height in m",
                                  'Fraction of hardwood \ndeciduous forest','Fraction of anthropo-\ngenic land',
                                  'Soil moisture index', "Sqrt-transformed effort \nas bird observations per \n100 ha"
                                  )) +
  ...
  theme(legend.position = "bottom" ) # c(0.85,0.22)
plot

# save marginal effects plot 
ggsave(filename = 'output/plots/Marsh.tit.marg.effects.effort.png', plot = plot, dpi = 600, height = 7, width = 8.5)
...
# set effort to 0 (mean since scaled)
data.map$Effort.sqrt.scaled <- 0
data.map$Effort.log.scaled <-0
...
# save map 
ggsave(filename = 'output/plots/Marsh.tit.pred.distribution.scania.effort.png', plot = pred.map, dpi = 1000, width = 8, height = 7)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 8. Export prediction map ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


# as geopackage 
st_write(data.map %>% select(-matches("\\.scaled$")), "output/Marh.tit.Occurence.Scania.predicted.effort.gpkg")
